...
class CardScanner extends EventEmitter {
    ...
    initialize() {
        try {
            ...
            const scannerDevice = devices.find(d => 
                d.vendorId === 0x08ff && d.productId === 0x0009 // Пример для некоторых считывателей карт
            );
            ...
            if (scannerDevice) {
                ...
                devices.forEach(d => {
                    console.log(`  - ${d.product} (VID: 0x${d.vendorId.toString(16)}, PID: 0x${d.productId.toString(16)})`);
                });
        ...
        this.device.on('data', (data) => {
            try {
                ...
                for (let i = 0; i < data.length; i++) {
                    ...
                    } else if (char.match(/[\x20-\x7E]/)) { // Печатные ASCII символы
